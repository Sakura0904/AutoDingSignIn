...
ui.layout(
    <scroll bg="#FFE7BA">
        <vertical padding="25">
            ...
            <text gravity="right" textSize="10sp" textColor="black" textStyle="bold"> 版本：V1.1.1</text>
            <vertical>
...
//var userData = storages.create();
/***********************************全局变量*********************************/
//User_DataSetUp();//用户数据本地储存返回
/***********************************UI界面选择线程*********************************/
    ...
    threads.start(function(){
        if(Check_the_ui()){
            ...
            if(confirm("是否确定开始定时签到功能")){//对话框确认开启
                alert("成功开启");
                log("User：主动启动定时程序");
                log("User：早间签到时间"+userTime.timeMorningHour+":"+userTime.timeMorningMin);
                log("User：午间签到时间"+userTime.timeNoonHour+":"+userTime.timeNoonMin);
                log("User：晚间签到时间"+userTime.timeEveningHour+":"+userTime.timeEveningMin);
                Sakura_MainTime();
            }
        }
    })
...
function Sakura_MainTime(){
    while(1){
        ...
        if(userUi.singinEvening && curTime.getHours()==21 && curTime.getMinutes()==22){
            log("Auto：测试签到时间到"+userTime.timeEveningHour+":"+userTime.timeEveningMin);
            Check_the_screen();
        }/*测试程序*/
...
function Check_the_ui(){
    ...
    else if(userUi.phoneKey1>=10 || userUi.phoneKey2>=10 || userUi.phoneKey3>=10 || 
...
function Unlock_Screen() {
    ...
    if(userUi.keynum==4){
        desc(userUi.phoneKey1).findOne().click();log("Procedure：已输入密码："+userUi.phoneKey1);
        desc(userUi.phoneKey2).findOne().click();log("Procedure：已输入密码："+userUi.phoneKey2);
        desc(userUi.phoneKey3).findOne().click();log("Procedure：已输入密码："+userUi.phoneKey3);
        desc(userUi.phoneKey4).findOne().click();log("Procedure：已输入密码："+userUi.phoneKey4);
    }
        ...
        desc(userUi.phoneKey3).findOne().click();log("Procedure：已输入密码："+userUi.phoneKey3);
        desc(userUi.phoneKey4).findOne().click();log("Procedure：已输入密码："+userUi.phoneKey4);
        desc(userUi.phoneKey5).findOne().click();log("Procedure：已输入密码："+userUi.phoneKey5);
        desc(userUi.phoneKey6).findOne().click();log("Procedure：已输入密码："+userUi.phoneKey6);
    }
    sleep(2000);//密码输入
    home();
    sleep(1000);//回到手机主页
